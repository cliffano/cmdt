...
Cmdt.prototype._exec = function (tests, cb) {
  ...
  function iter(test, cb) {
    ...
    var _exec = child.exec(test.command, { cwd: testDir }, self._testCb(test, exitCode, output, cb));
 ...
 * @param {String} output: test output string, combining both stdout and stderr
 * @param {Function} cb: standard cb(err, result) callback
 */
Cmdt.prototype._testCb = function (test, exitCode, output, cb) {
  return function (err) {
    if (!err) {
