...
function Cmdt(opts) {
  ...
  this.opts.baseDir = opts.baseDir || os.tmpdir();
...
Cmdt.prototype.run = function (files, cb) {
  const FIELDS = ['command', 'exitcode', 'output'];
  var self = this;
  ...
  async.eachSeries(files, iter, function (err, results) {
    if (!err) {
      ...
      if (!self.opts.debug) {
        rimraf.sync(p.join(self.opts.baseDir, self.runId));
      }
    }
    cb(err, results);
  });
...
Cmdt.prototype._exec = function (tests, cb) {
  ...
  function iter(test, cb) {
    ...
    var output = '',
      dir = p.join(self.opts.baseDir, self.runId, test.file.replace(/.cmdt$/, ''));

    mkdirp.sync(dir);

    var _exec = child.exec(test.command, { cwd: dir }, _execCb);
