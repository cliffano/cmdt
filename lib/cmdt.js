 ...
 * @param {Array} fixtures: test fixtures directories, to be copied to test execution directory
 * @param {Function} cb: standard cb(err, result) callback
 */
Cmdt.prototype._exec = function (tests, fixtures, cb) {
    ...
    function execTestCb(err) {
      if (err) {
        ...
        _exec.stdout.on('data', function (data) {
          ...
          self._execData[test.file].stdout += data;
        });
        _exec.stderr.on('data', function (data) {
          self._execData[test.file].output += data;
          self._execData[test.file].stderr += data;
        });
      }
    }

    function copyFixtures(fixtureDir, cb) {
      ncp.ncp(fixtureDir, testDir, cb);
    }

    async.each(fixtures, copyFixtures, execTestCb);
  }
...
Cmdt.prototype._testCb = function (test, cb) {
  ...
  return function (err) {
    if (!err) {
      var result = self._execData[test.file];
